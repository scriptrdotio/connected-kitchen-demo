var config =  require("/smartkitchens/entities/config").zoho;
...
zoho.prototype.createTicket = function(contactName, contactEmail, subject, message, deviceId, status, department){
  if(!department){
    ...
    log.info("Ticket zoho: "+JSON.stringify(params))
...
zoho.prototype.getrecordsbysearch = function(fieldName, fieldValue, department){
  ...
  var url ="https://support.zoho.com/api/json/requests/getrecordsbysearch?authtoken="+ config.supportAuthToken +"&portal="+ config.portal+ "&department="+ encodeURIComponent(department) +"&searchfield="+ encodeURIComponent(fieldName) +"&searchvalue=" +encodeURIComponent(fieldValue)+"&selectfields="+encodeURIComponent("Requests(created Time,status,Account Name,ACCOUNTID,Request Id,Ticket Id,Subject,Description,Phone,Email,Contact Name,CONTACTID,Department,DEP_ID");
log.info(url)
